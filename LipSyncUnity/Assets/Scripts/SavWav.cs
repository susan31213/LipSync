...
public static class SavWav
{
    ...
    public static void SaveBytes(string filename, byte[] wavBytes, uint length, bool trim = false)
    {
        if (!filename.ToLower().EndsWith(".wav"))
        {
            filename += ".wav";
        ...
        using (var writer = new BinaryWriter(fileStream))
        {
            writer.Write(wavBytes, 0, (int)length);
            Debug.Log("Saved at: " + filepath);
        }
    }
    ...
    public static byte[] GetWavWithoutHeader(AudioClip clip, out uint length, bool trim = false)
    {
        var data = ConvertAndWrite(clip, out length, out var samples, trim);
        return data;
    }
    ...
    public static void WriteHeader(byte[] stream, AudioClip clip, uint length, uint samples)
    {

...
class UdpComms():
    ...
    def SendStrData(self, strToSend):
        # Use this function to send string to C#
        self.udpSock.sendto(bytes(strToSend,'utf-8'), (self.udpIP, self.udpSendPort))
    
    def SendUintData(self, uintToSend):
        # Use this function to send string to C#
        self.udpSock.sendto(bytes(uintToSend), (self.udpIP, self.udpSendPort))
    ...
    def ReceiveByteData(self):
        ...
        data = None
        try:
            data, _ = self.udpSock.recvfrom(32000)
        except WindowsError as e:
            if e.winerror == 10054: # An error occurs if you try to receive before connecting to other application
                if not self.suppressWarnings:
                    print("Are You connected to the other application? Connect to it!")
    ...
    def ReadUdpThreadFunc(self): # Should be called from thread
        ...
        while True:
            data = self.ReceiveByteData()  # Blocks (in thread) until data is returned (OR MAYBE UNTIL SOME TIMEOUT AS WELL)
            self.dataRX.put(data) # Populate AFTER new data is received
    ...
    def ReadReceivedData(self):
        ...
        if self.dataRX.qsize() > 0:
            # self.isDataReceived = False
            data = self.dataRX.get()
